...
## Real LLM agent runner (PR #10 and later)

Opt-in via `FURNACE_AGENT_RUNNER=llm` (requires `FURNACE_MASTER_ENCRYPTION_KEY` so a `ModelRouter` can be built). This replaces the scripted `FakeAgentRunner` with real BYOK LLM calls; tools still dispatch through the (fake) provisioner, so file reads/builds return canned results — this proves the live LLM→tool→event→persist wiring, not a real iOS build.

- **Provider MUST match the stored BYOK key.** The New Session form defaults to provider `openrouter`; if your BYOK key is `openai`, explicitly select Provider=openai (then the Model dropdown switches to the openai catalog, e.g. `gpt-4o-mini`). A mismatch makes the router fail to find a key. `buildDefaultModelPolicy(provider, model)` in `apps/web` builds the policy from the selected provider.
- Create the BYOK key first via `POST /v1/keys` (provider=openai, the raw secret). Confirm with `GET /v1/keys` (metadata only).
- **Restart the backend after code changes** — uvicorn is launched without `--reload`, so a running server keeps stale code (this is how a fixed bug can appear "unfixed"). `kill` the pid and relaunch.
- Golden path: `agent_plan` (real natural-language plan referencing the prompt) → model-chosen `list_files`/`read_file`/`write_file`/`run` actions each with an `agent_observation` → `agent_message` → `agent_result` `success:true`, then SUCCEEDED. A `write_file` action's `content` is genuinely model-authored code — that (plus the model-authored plan) is what distinguishes it from the fixed FakeAgentRunner script (`read_file → apply_patch → xcodebuild → run_ui_test`).
- **Adversarial usage check (regression guard for the duplicate-UsageRecord bug):** query `usage_records` for the session — expect exactly ONE row with `prompt_tokens>0` AND `completion_tokens>0` (from the router's token metering) AND `mac_seconds>0` (from the orchestrator). Both writers must coexist on one row. If the orchestrator's `_create_usage` were an unconditional INSERT again, the run would end FAILED with `UNIQUE constraint failed: usage_records.session_id` and 0 rows/`error` event instead of SUCCEEDED. Query by the dash-less session id (SQLite stores UUIDs without dashes).
- The Usage panel in the UI may still show "Usage not available yet" after completion (it doesn't refetch usage post-run) — verify usage in the DB, not the panel.
